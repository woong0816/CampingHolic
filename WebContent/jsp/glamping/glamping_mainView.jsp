				...
				<br>
					<div class="paging">
						<c:if test="${viewData.startPage !=1 }">
							<a class="direction fisrt" href = "glamping_main?page=1"
							...
							>&lt; 처음</a>
							<a class="direction prev" href = "glamping_main?page=${viewData.startPage-1}">&lt; 이전</a>
								<c:if test="${viewData.type != null}">&type=${viewData.type}</c:if>
								<c:if test="${viewData.keyword != null}">&keyword=${viewData.keyword}</c:if>
								<c:if test="${viewData.filter!=null}">&filter=${viewData.filter}</c:if>
						</c:if>
						...
						<c:if test = "${viewData.endPage < viewData.pageTotalCount}">
							<a class="direction next" href = "glamping_main?page=${viewData.endPage+1}
								<c:if test="${viewData.type != null}">&type=${viewData.type}</c:if>
								<c:if test="${viewData.keyword != null}">&keyword=${viewData.keyword}</c:if>
								<c:if test="${viewData.filter!=null}">&filter=${viewData.filter}</c:if>
							
							">다음 &gt;</a>
							<a class="direction last" href = "glamping_main?page=${viewData.pageTotalCount}
								<c:if test="${viewData.type != null}">&type=${viewData.type}</c:if>
								<c:if test="${viewData.keyword != null}">&keyword=${viewData.keyword}</c:if>
								<c:if test="${viewData.filter!=null}">&filter=${viewData.filter}</c:if>
							">&nbsp;끝&nbsp; &gt;</a>
