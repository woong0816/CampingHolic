...
<script type="text/javascript">
	var ac_num = ${detailView.autoCampingBoard.AC_NUM};
	var m_num = <%=session.getAttribute("memberNum")%>
	var board_memberNum = ${detailView.autoCampingBoard.M_NUM};
	...
	$(function(){
		...
		$("#replyForm").on("submit",function(){
			...
			$.ajax({
				url : "${contextPath}/autoCampingReply/replyInsert",
				data : { "ac_num" : ac_num,
						 "m_num" : m_num,
	...
	function getReplyList(){
		...
		$.ajax({
			url : "${contextPath}/autoCampingReply/getReplyList",
			data : {"ac_num" : ac_num},
			type : "get",
			dataType : "json",
			success : function(data){
	...
	function deleteBtn(rNum){
		...
		$.ajax({
			url : "${contextPath}/autoCampingReply/deleteReply",
			data : {"abr_num" : rNum,
					"ac_num" : ac_num,
					"m_num" : m_num
					},
	...
	function board_delete(){
		if(confirm("삭제 하시겠습니까?")){
			location.href = 'autoCampingDelete?ac_num=${detailView.autoCampingBoard.AC_NUM}';
		}else{
			return;
		}
	...
	function like_func(){
		$.ajax({
			url : "${contextPath}/autoCamping/checkLike",
			data : { "ac_num" : ac_num,
					"m_num" : m_num
...
<body>
	...
	<div style="text-align:center;"> 
		<img src="${contextPath}/img/autoCamping_main.jpg" style="width: 1880px; height:500px;"></img>
	</div>

	<div class="container">
		<br><br><br><br><br><br>
			
			<div><span id="title">${detailView.autoCampingBoard.AC_TITLE}</span></div>
			<div class="write_na">작성자: ${detailView.autoCampingBoard.NICKNAME}
				<div style="display: inline-block; text-align: right; float: right;">
					<span style="margin-right: 180px;">조회 수: ${detailView.autoCampingBoard.READCOUNT}</span>
				</div>
			</div>
				<div class="write_date">
					<fmt:parseDate var="parseRegDate" value="${detailView.autoCampingBoard.WRITEDATE}" pattern="yyyy-mm-dd"/>
	...
	<div>
		<div class="map_box" id="map" style="width:50%; height:420px; float:right;"></div>
	</div>
		<div class="address_box"><span id="address">${detailView.autoCampingBoard.AC_ADDRESS}</span></div>
		...
		<script>
			    ...
			    geocoder.addressSearch('${detailView.autoCampingBoard.AC_ADDRESS}', function(result, status) {
	...
	<div style="width: 100%; height: 380px; display: table-cell;">		
		<div class="w3-content w3-display-container" id="display-container" style="height: 434px; width:680px; clear: both; float: left;">
				<c:forEach items="${detailView.autoCamping_fileList}" var ="fileList">
					<img src="${contextPath}${fileList.ACI_IMGFILENAME}">
				</c:forEach>
	...
	<div style="text-align: center;">
		<span class="userNameText" style="font-size: 17px; font-weight: bold;">${nickName}</span>
			<form name="replyForm" id="replyForm">
				...
				<input type="submit" name="reply_btn" id="reply_btn" style="border: solid 1px #1f8dd6; background-color: #1f8dd6; color: white; width: 70px; height: 80px;" value="등록">
			</form>
